...
int main() {
    ...
    while (t--) {
        ...
        LL ans = 0;
        for (const auto& [x, res] : cnt) {
            ans += 1ll * res * (res - 1) * (res - 2);
        }
        for (int i = 1; i <= n; i++) {
            if (a[i] > 1000000) {
                for (int d = 2; d <= 1000 and 1ll * a[i] * d <= 1000000000; d++) {
                    if (a[i] % d) continue;
                    int x = a[i] / d, y = a[i] * d;
                    auto xx = cnt.find(x);
                    auto yy = cnt.find(y);
                    if (xx != cnt.end() and yy != cnt.end()) {
                        ans += 1ll * (*xx).ss * (*yy).ss;
                    }
                }
            } else {
                for (int d = 1; d * d <= a[i] and 1ll * a[i] * d <= 1000000000; d++) {
                    if (a[i] % d) continue;
                    if (d != 1 and 1ll * a[i] * d <= 1000000000) {
                        int x = a[i] / d, y = a[i] * d;
                        auto xx = cnt.find(x);
                        auto yy = cnt.find(y);
                        if (xx != cnt.end() and yy != cnt.end()) {
                    ...
                    int dd = a[i] / d;
                    if (dd != d and dd != 1 and 1ll * a[i] * dd <= 1000000000) {
                        int x = a[i] / dd, y = a[i] * dd;
                        auto xx = cnt.find(x);
                        auto yy = cnt.find(y);
                        if (xx != cnt.end() and yy != cnt.end()) {
                            ans += 1ll * (*xx).ss * (*yy).ss;
